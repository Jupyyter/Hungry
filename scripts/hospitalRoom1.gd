	...
	match npcConv:
		0:
			text_box.queue_text("why do we have a homeless eating Tom in the hospital?--color1
				sir, he is mister Gabriel, he is renowned for eating a brick wall, can i keep him?--color2
				gabriel, sit--color1")
			npcConv+=1
		1:
			if textReady():
				...
				text_box.queue_text("good gabriel--color1
					now bark--color1")
				text_box.queue_questionResponse("wof")
				text_box.queue_text("you can keep him--color1
					thank you sir--color2")
				npcConv+=1
		2:
			if textReady():
